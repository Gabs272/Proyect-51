...
if(zombieGroup.isTouching(player)){
 ...
 for(var i=0;i<zombieGroup.length;i++){     
  ...
  if(zombieGroup[i].isTouching(player)){
       //zombieGroup[i].bounceoff(player);
       //zombieGroup[i].collide(player);
...
function enemy(){
  if(frameCount%50===0){
    ...
    zombie.x = Math.round(random(windowWidth-100,windowWidth-600));
    
    zombie.y = Math.round(random(windowHeight-100,windowHeight-500));
